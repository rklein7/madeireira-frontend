...
export interface PedidoCompraPDFItem {
...
export interface PedidoCompraPDFProps {
  pedido: {
    numero: string
    status: string
    ...
    fornecedorBairro?: string
    fornecedorCidade?: string
    fornecedorUf?: string
    fornecedorCep?: string
    fornecedorTelefone?: string
    fornecedorEmail?: string
    usuarioNome?: string
    itens: PedidoCompraPDFItem[]
  }
  empresa?: {
    nome: string
    cnpj: string
...
export function PedidoCompraPDF({ pedido, empresa }: PedidoCompraPDFProps) {
  ...
  const enderecoLinha1 = [pedido.fornecedorEndereco, pedido.fornecedorBairro]
    .filter(Boolean)
    .join(', ')
  const cidadeUf =
    pedido.fornecedorCidade && pedido.fornecedorUf
      ? `${pedido.fornecedorCidade}/${pedido.fornecedorUf}`
      : (pedido.fornecedorCidade ?? pedido.fornecedorUf)
  const enderecoLinha2 = [pedido.fornecedorCep, cidadeUf]
    .filter(Boolean)
    .join('   ·   ')
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.headerWrapper}>
          <View style={styles.headerRow}>
            ...
            <View style={styles.headerDireita}>
              <Text style={styles.pedidoTitulo}>Pedido de compra</Text>
        ...
        <View style={styles.clienteRow}>
          ...
          <View style={[styles.clienteColuna, styles.vendedorColuna]}>
            <Text style={styles.clienteLinha}>
              <Text style={styles.clienteLabel}>Comprador: </Text>
              {pedido.usuarioNome ?? '—'}
            </Text>
          </View>
        </View>
        ...
        <Text style={styles.paragrafo}>
          Previsão de entrega: {formatData(pedido.previsaoEntrega)}
        </Text>
        ...
        <View style={styles.rodape} fixed>
          ...
          <Text style={styles.rodapeTexto}>
            Pedido de compra — sujeito a confirmação do fornecedor
          </Text>
        </View>
      </Page>
    </Document>
